...
export default function App() {
  ...
  const [walletFullAddress, setWalletFullAddress] = useState<string>(""); // Store full public key for Live Mode
  ...
  // Stamp Collection Achievement System
  ...
  // Calculate unique stamps and check achievement status
  useEffect(() => {
    // Calculate unique cardIndex values
    ...
    // Load achievement status from LocalStorage
    const savedAchievement = localStorage.getItem("stamp_achievement_unlocked");
    const wasUnlocked = savedAchievement === "true";
    
    // Check if achievement should be unlocked
    if (uniqueCards.size >= 5 && !wasUnlocked) {
      setAchievementUnlocked(true);
      localStorage.setItem("stamp_achievement_unlocked", "true");
      triggerNotification("🎉 Achievement Unlocked! You've collected 5 unique stamps! Premium theme now available.");
    } else if (wasUnlocked) {
      setAchievementUnlocked(true);
    }

    // Load theme preference
    const savedTheme = localStorage.getItem("premium_theme_active");
    if (savedTheme === "true" && wasUnlocked) {
      setPremiumThemeActive(true);
    }
  }, [mailbox]);
  ...
  const handleConnectWalletBtn = async () => {
    if (walletConnected) {
      ...
      // Live Mode: Use real Phantom wallet
      if (simulationMode === "live") {
        await handleLiveWalletConnect();
      } else {
        // Simulate Mode: Show mock modal
  ...
  // Live Mode: Real Phantom wallet connection
  const handleLiveWalletConnect = async () => {
    try {
      if (!isPhantomInstalled()) {
        triggerNotification("❌ Phantom wallet not detected. Please install it from phantom.app");
        window.open("https://phantom.app/", "_blank");
        return;
      }

      setWalletIsLoading(true);
      triggerNotification("🔗 Connecting to Phantom wallet...");

      const { publicKey, address } = await connectPhantomWallet();
      
      setWalletConnected(true);
      setWalletFullAddress(publicKey);
      setWalletAddress(address);
      setWalletIsLoading(false);
      
      triggerNotification("✅ Phantom wallet connected successfully!");
    } catch (error: any) {
      setWalletIsLoading(false);
      console.error("Wallet connection error:", error);
      triggerNotification(`❌ Connection failed: ${error.message || "Unknown error"}`);
    }
  };

  const executeDisconnect = async () => {
    try {
      // Live Mode: Real disconnect
      if (simulationMode === "live" && walletFullAddress) {
        await disconnectPhantomWallet();
        triggerNotification("👛 Phantom wallet disconnected.");
      } else {
        // Simulate Mode
      ...
      setWalletFullAddress("");
      setWalletAddress("SoFtJn_6zRxF3X8yQwPqL");
      setShowWalletModal(false);
    } catch (error: any) {
      console.error("Disconnect error:", error);
      triggerNotification(`❌ Disconnect failed: ${error.message || "Unknown error"}`);
    }
  };
  ...
  // Now also accepts real Solana signatures from Live Mode
  const getDeterministicHash = (name: string, message: string, amount: number, realSignature?: string) => {
    // If we have a real signature from Live Mode, use it directly
    if (realSignature) {
      // Extract seed value from real signature for deterministic card selection
      let hashCode = 0;
      for (let i = 0; i < Math.min(realSignature.length, 32); i++) {
        hashCode = (hashCode << 5) - hashCode + realSignature.charCodeAt(i);
        hashCode |= 0;
      }
      const absHash = Math.abs(hashCode);
      const cardIndex = (absHash % 34) + 1;
      
      return { txSig: realSignature, seedVal: absHash, cardIndex };
    }
    
    // Simulate Mode: Generate deterministic mock signature
  ...
  const handleSendTip = async (e: React.FormEvent) => {
    ...
    // Live Mode: Execute real blockchain transaction
    if (simulationMode === "live") {
      await handleLiveTransaction();
    } else {
      // Simulate Mode: Show approval modal
      setWalletModalMode("approve");
      setShowWalletModal(true);
    }
  };

  // Live Mode: Real Solana transaction
  const handleLiveTransaction = async () => {
    const finalAmount = getSelectedAmount();
    
    try {
      setIsBrewing(true);
      triggerNotification("🔗 Sending transaction to Solana Devnet...");

      // Send real SOL transaction
      const { signature, amount } = await sendSolTransaction(finalAmount);
      
      triggerNotification("✅ Transaction confirmed on Solana Devnet!");
      
      // Process the real signature through our existing system
      await processTransactionSuccess(signature, finalAmount, true);
      ...
      if (error.message?.includes("User rejected")) {
        triggerNotification("❌ Transaction cancelled by user.");
      } else if (error.message?.includes("insufficient")) {
        triggerNotification("❌ Insufficient SOL balance. Get Devnet SOL from faucet.solana.com");
      } else {
        triggerNotification(`❌ Transaction failed: ${error.message || "Unknown error"}`);
      }
    }
  };

  // Process transaction success (works for both Live and Simulate modes)
  const processTransactionSuccess = async (signature: string, finalAmount: number, isLiveMode: boolean = false) => {
    // Retrieve deterministic data using real or mock signature
    const { txSig, seedVal, cardIndex } = getDeterministicHash(
      fanName, 
      fanMessage, 
      finalAmount,
      isLiveMode ? signature : undefined
    );
    ...
    if (isLiveMode) {
      triggerNotification("📓 Real blockchain receipt postcard minted!");
    } else {
  ...
  const finalizeTipPostcardMint = () => {
    ...
    setTimeout(async () => {
      await processTransactionSuccess("", finalAmount, false);
    }, 1200);
  };
  ...
  // Module C: Twitter Share Intent Feature
  const shareToTwitter = () => {
    if (!activePostcard) return;

    try {
      // Get rarity badge text
      const rarityBadge = getRarityBadge(activePostcard.rarity);
      
      // Format transaction hash for display
      const shortTxHash = activePostcard.txHash.length > 20 
      ...
      const tweetText = `Just supported creator @lijency76 on SoftPay with a cozy tea break! 🍵

Scratched off a rare ${rarityBadge.text} watercolor stamp! 🎴

Check out my on-chain signature seed: ${shortTxHash}

Collect and scratch your own stamp here: ${productionUrl}`;

      // Encode URI and open Twitter Intent
      ...
      triggerNotification("📢 Share intent opened! Don't forget to attach your downloaded postcard image to amaze your followers!");
      
    } catch (error) {
      console.error('Twitter share error:', error);
      triggerNotification("❌ Failed to open share dialog. Please try again.");
    }
  };

  // Module D: Deterministic Postcard Rotation Physics
  const getPostcardRotation = (txHash: string): number => {
    // Extract deterministic seed from transaction hash
    let rotationSeed = 0;
    for (let i = 0; i < Math.min(txHash.length, 16); i++) {
      rotationSeed += txHash.charCodeAt(i) * (i + 1);
    }
    
    // Map to range between -3 and +3 degrees for organic paper physics
    const normalizedRotation = (rotationSeed % 600) / 100; // 0 to 6
    const finalRotation = normalizedRotation - 3; // -3 to +3
    
    return finalRotation;
  };

  return (
    <div className={`min-h-screen text-black font-sans relative flex flex-col justify-between py-12 px-6 md:px-12 selection:bg-black selection:text-white transition-colors duration-500 ${
      premiumThemeActive ? 'bg-gradient-to-br from-[#FFF5F7] via-[#FFF9E6] to-[#F0F8FF]' : 'bg-white'
    }`}>
      ...
      <div className="w-full max-w-5xl mx-auto flex-1 flex flex-col justify-start">
              ...
              <div className="flex gap-1 border-2 border-black rounded-lg p-0.5 bg-white shadow-[1px_1px_0px_0px_#000000]">
                ...
                <button 
                  onClick={() => {
                    ...
                    triggerNotification("🌐 Solana Devnet Live Mode activated. Real transactions enabled!");
                  }}
                  className={`py-1 px-2.5 text-[10px] uppercase font-extrabold tracking-wider transition-all rounded-md cursor-pointer ${
        ...
        <div id="sketchbook-mailbox" className="notion-card p-6 mb-12 flex flex-col justify-between">
          ...
          {/* Stamp Collection Progress Tracker */}
          <div className={`mb-6 p-4 rounded-xl border-2 transition-all duration-500 ${
            premiumThemeActive 
              ? 'bg-gradient-to-r from-pink-50 to-purple-50 border-purple-300' 
              : 'bg-[#F4F4F4] border-black'
          }`}>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-bold uppercase tracking-wider text-black flex items-center gap-2">
                    ...
                    {achievementUnlocked && (
                      <span className="text-[9px] bg-amber-400 text-amber-900 px-2 py-0.5 rounded-full font-black border border-amber-600">
                        ✦ COLLECTOR
                      </span>
                    )}
                  </h4>
                  <p className="text-xs text-gray-600 mt-0.5 font-medium">
                    Collect unique cards to unlock achievements
                  </p>
                </div>
              </div>
                ...
                {achievementUnlocked && (
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg text-xs font-bold transition-all border-2 ${
                      premiumThemeActive
                        ? 'bg-gradient-to-r from-pink-400 to-purple-400 text-white border-purple-500 shadow-lg'
                        : 'bg-white text-black border-black hover:bg-gray-50'
                    }`}
                  >
                    {premiumThemeActive ? '✨ Premium' : '🎨 Theme'}
                  </button>
                )}
              </div>
            </div>
            ...
            {/* Progress Bar */}
            <div className="mt-4 w-full bg-gray-200 rounded-full h-2 overflow-hidden border border-black/20">
              <div 
                ...
                style={{ width: `${(uniqueStampsCount / 34) * 100}%` }}
              />
            </div>
          </div>
          ...
          >
            <motion.div
              ...
              animate={{ 
                y: 0, 
                opacity: 1, 
                rotate: getPostcardRotation(activePostcard.txHash), // Deterministic rotation based on TxHash
                scale: 1 
              }}
            ...
            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
                  ...
                  <div className="flex flex-col justify-between text-left space-y-4">
                    ...
                    <div className="py-4 flex items-center justify-center">
                      <div className="border-2 border-dashed border-black rounded-lg p-2 bg-white/50">
                        <ScratchCard
              ...
              <div className="mt-6 grid grid-cols-3 gap-3">
                <button
                  ...
                  className="flex items-center justify-center gap-2 py-4 bg-white border-2 border-black text-black hover:bg-gray-50 rounded-xl text-xs tracking-wide uppercase font-black cursor-pointer transition-all shadow-[3px_3px_0px_0px_#000000] hover:shadow-[5px_5px_0px_0px_#000000]"
                >
                  <span>💾</span>
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 py-4 bg-[#1DA1F2] border-2 border-black text-white hover:bg-[#1a8cd8] rounded-xl text-xs tracking-wide uppercase font-black cursor-pointer transition-all shadow-[3px_3px_0px_0px_#000000] hover:shadow-[5px_5px_0px_0px_#000000]"
                >
                ...
                <button
                  type="button"
                  onClick={() => setShowPostcard(false)}
                  className="notion-btn-primary py-4 hover:bg-gray-900 justify-center text-xs tracking-wide uppercase font-black cursor-pointer"
                >
                  <span className="hidden sm:inline">Done & Return</span>
